...
const HeroSection = () => {
  ...
  const [progressLine, setProgressLine] = useState([]); // Stores points for the zig-zag line
  ...
  const handleAnswerSubmit = () => {
    ...
    if (userAnswer.trim().toLowerCase() === correctAnswer.toLowerCase()) {
      ...
      setTimeout(() => {
        ...
        // Update the zig-zag line progress
        setProgressLine((prevLine) => [
          ...prevLine,
          { x: characterPosition * 100 + 50, y: 200 - barHeights[characterPosition] }, // Add a new point to the line
        ]);
        ...
        <div className="mt-12">
          ...
          <div className="mt-8 w-full h-64 relative flex items-end justify-center space-x-4 border-b-2 border-[#170F00]">
            ...
            {barHeights.map((height, index) => (
              <div
                key={index}
                className="w-12 bg-[#FAA51D] relative rounded-t-lg shadow-lg"
                ...
                {index === characterPosition && (
                  ...
                  >
                    <motion.img
                      ...
                      className="w-16 h-16"
                      animate={{
                        rotate: isJumping ? [0, -10, 10, 0] : 0, // Leg movement animation
                      }}
                      transition={{
                        duration: 0.5,
                        ease: "easeInOut",
                        repeat: isJumping ? Infinity : 0, // Repeat while jumping
                      }}
                    />
                  </motion.div>
                )}
            ...
            <svg
              ...
              viewBox="0 0 400 200" // Adjust viewBox to match the bar chart dimensions
              preserveAspectRatio="none"
            >
              <polyline
                points={progressLine
                  .map((point) => `${point.x},${point.y}`) // Convert points to SVG coordinates
                  .join(" ")}
                fill="none"
                stroke="#FAA51D"
                strokeWidth="2"
                strokeDasharray="5,5" // Dashed line for a zig-zag effect
              />
            </svg>
          </div>
        </div>
